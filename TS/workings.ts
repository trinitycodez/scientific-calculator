let work_class = document.querySelector(".work-class")! as HTMLDivElement
let result_class = document.querySelector(".result-class")! as HTMLDivElement
...
// holder solve
let holderOprt:string = "";
let holderNum:string = "";
let solver:string = "";

function solve(value:HTMLDivElement, oprt?:string) {
    equatt(value);
    let span = "<span>";
    if (oprt && value.textContent) {
        ...
        if (y == doc4) y = `${y + doc6}`;
        if (y == doc9) y = y + "^(";
        if ((y == doc10 ) || (y == doc12) || (y == doc13) || (y == doc17) || (y == doc18) || (y == doc19)) y = y + "(";
        if (y == doc15) y = "<sup>2</sup>";
        if (y == doc16) y = "<sup>3</sup>";
        span += y + "</span>";
        holderOprt = span;
...
const emptyItr:string[] = []; // -- for square root manipulation --
const pii_array:string[] = []; // -- for Pii manipulation --
const exp_array:string[] = []; // -- for exponetial power manipulation --
const power_array:string[] = []; // -- for power ^ manipulation --
const factorial_array:string[] = []; // -- for factorial ! manipulation --
const LN_array:string[] = []; // -- for Natural Logarithm manipulation --
const L10_array:string[] = []; // -- for Logarithm base 10 manipulation --
const Ans_array:string[] = []; // -- for ANSwer manipulation --
const power2_array:string[] = []; // -- for Power2 manipulation --
const sohCahToa:string[] = []; // -- for sohCahToa manipulation --
const percentage_array:string[] = []; // -- for percentage manipulation --

const factorial_pack:string[] = [];
let factorial_val = "";
const percentage_pack:string[] = [];
let percentage_val = "";
...
function equatt(valTxt:HTMLDivElement) {
    ...
    switch (y) {
        ...
        case arr2[4]:
            ...
            if (!solver.valueOf()) { solver = "Math.sqrt("; emptyItr.push("M"); break; }
        ...
        case arr2[6]:
            //doc6 open bracket
            oddEven();
            if (!solver.valueOf()) { solver = "("; break; }
        ...
        case arr2[8]:
            //doc8 Pii
            if (!solver.valueOf()) { solver = "Math.PI"; pii_array.push("codeNum"); break;}
            if (solver.endsWith(OPRT.PLUS)) {
                solver += "Math.PI";
        ...
        case arr2[9]:
            //docs 9 exponetial e
            if (!solver.valueOf()) {solver = "Math.exp("; exp_array.push("M"); break;}
        ...
        case arr2[10]:
            ...
            if (!solver.valueOf()) {solver = "Math.pow(,"; break;}
            if ((solver.includes("(") && solver.includes(")")) && solver.endsWith(")")) {
                ...
                if (solver.endsWith("Math.pow")) { solver += `(Math.pow${subStr},`; console.log("falsify"); break; } // (Math.pow(2,3)...
        ...
        case arr2[11]:
            ...
            if ((solver.includes("(") && solver.includes(")")) && solver.endsWith(")")) {
                ...
                if (i > p || i == p) {
                    ...
                    if (solver.endsWith("Math.sqrt")) {solver += `(${hold1})`; console.log(solver + " falsify"); return;} // for math sqrt
                    solver += `${hold1}`;
        ...
        case arr2[12]:
            ...
            if (!solver.valueOf()) { solver = "Math.log("; LN_array.push("M"); break; }
        ...
        case arr2[13]:
            ...
            if (!solver.valueOf()) {solver = "Math.log10("; L10_array.push("M"); break; }
            if (solver.endsWith(OPRT.PLUS) || solver.endsWith(OPRT.MINUS) || solver.endsWith(OPRT.DIVISION) || solver.endsWith(OPRT.MULTIPLE) || solver.endsWith("(") ) {
                solver += "Math.log10(";
                L10_array.push("M");
            } else {
        ...
        case arr2[14]:
            ...
            if (!solver.valueOf()) { solver = "Ans"; break; }
            if (solver.endsWith(OPRT.PLUS) || solver.endsWith(OPRT.MINUS) || solver.endsWith(OPRT.DIVISION) || solver.endsWith(OPRT.MULTIPLE) || solver.endsWith("(") ) {
                solver += "Ans";
                Ans_array.push("A");
                break;  
        ...
        case arr2[15]:
            ...
            if (!solver.valueOf()) { solver = "Math.pow(,2)"; break; }
            if ((solver.includes("(") && solver.includes(")")) && solver.endsWith(")")) {
                solver += "2";
                ...
                let subStr = solver.substring(opb, pwr);
                solver = solver.substring(0, opb);
                if (solver.endsWith("Math.pow")) { solver += `(Math.pow${subStr},2)`; console.log("falsify"); break; }
        ...
        case arr2[16]:
            ...
            if (!solver.valueOf()) { solver = "Math.pow(,3)"; break; }
            if ((solver.includes("(") && solver.includes(")")) && solver.endsWith(")")) {
                solver += "3";
                ...
                let subStr = solver.substring(opb, pwr);
                solver = solver.substring(0, opb);
                if (solver.endsWith("Math.pow")) { solver += `(Math.pow${subStr},3)`; console.log("falsify"); break; }
        ...
        case arr2[17]:
            // docs 17 of SinX...
            if (!solver.valueOf()) { solver = "Math.sin(Math.PI/180*"; break; }
            if (solver.endsWith(OPRT.PLUS) || solver.endsWith(OPRT.MINUS) || solver.endsWith(OPRT.DIVISION) || solver.endsWith(OPRT.MULTIPLE) || solver.endsWith("(")) {
        ...
        case arr2[18]:
            // docs 18 of CosX...
            if (!solver.valueOf()) { solver = "Math.cos(Math.PI/180*"; break; }
            if (solver.endsWith(OPRT.PLUS) || solver.endsWith(OPRT.MINUS) || solver.endsWith(OPRT.DIVISION) || solver.endsWith(OPRT.MULTIPLE) || solver.endsWith("(")) {
        ...
        case arr2[19]:
            // docs 19 of TanX...
            if (!solver.valueOf()) { solver = "Math.tan(Math.PI/180*"; break; }
            if (solver.endsWith(OPRT.PLUS) || solver.endsWith(OPRT.MINUS) || solver.endsWith(OPRT.DIVISION) || solver.endsWith(OPRT.MULTIPLE) || solver.endsWith("(")) {
        ...
        case arr2[20]:
            ...
            if ((solver.includes("(") && solver.includes(")")) && solver.endsWith(")")) {
                ...
                if (i > p || i == p) {
                    ...
                    if (solver.endsWith("Math.sqrt")) {solver += `(${hold1})`; return;} // for Math sqrt static func
                    solver += `${hold1}`;
                    return;
                }
...
function result() {
    ...
    try {
        if (solver) {
            let winDox = window.localStorage.getItem("ans");
            let result_val:number = eval((solver.replace(/Ans/gi, `${winDox}`)));
            let result_valStr:string = "";
...
function backSpace() {
    ...
    if (power2and3?.endsWith("<span><sup>2</sup></span>")) {
        // this solves for the power 2 and 3
        if (solver.endsWith(",2)")) {
            let hold1 = solver.lastIndexOf("(");
            let hold2 = solver.lastIndexOf(",");
            let hold1and2 = solver.substring(hold1+1, hold2);
            if (solver.endsWith("),2)")) {hold1and2 = solver.substring(hold1+1, hold2); console.log("falsify");}
            let hold3 = solver.lastIndexOf("Math.pow(");
            solver = solver.substring(0, hold3) + hold1and2;
        }
        console.log(solver + " solver");
    ...
    if (power2and3?.endsWith("<span><sup>3</sup></span>")) {
        // this solves for the power 2 and 3
        if (solver.endsWith(",3)")) {
            let hold1 = solver.lastIndexOf("(");
            let hold2 = solver.lastIndexOf(",");
            let hold1and2 = solver.substring(hold1+1, hold2);
            if (solver.endsWith("),3)")) {hold1and2 = solver.substring(hold1+1, hold2); console.log("falsify");}
            let hold3 = solver.lastIndexOf("Math.pow(");
            solver = solver.substring(0, hold3) + hold1and2;
            console.log(solver + " solver");
        }
    ...
    if (power2and3?.endsWith("<span>sin(</span>") || power2and3?.endsWith("<span>cos(</span>") || power2and3?.endsWith("<span>tan(</span>")) {
        let p, q = "";
        ...
        if ((solver.endsWith("*Math.sin(Math.PI/180*") || solver.endsWith("*Math.cos(Math.PI/180*") || solver.endsWith("*Math.tan(Math.PI/180*")) && (sohCahToa[sohCahToa.length-1] == "*")) {
            if (solver.endsWith("*Math.sin(Math.PI/180*")) { p = solver.lastIndexOf("*Math.sin(")}
            else if (solver.endsWith("*Math.cos(Math.PI/180*")) { p = solver.lastIndexOf("*Math.cos(")}
            else if (solver.endsWith("*Math.tan(Math.PI/180*")) { p = solver.lastIndexOf("*Math.tan(")}
    ...
    if (factorial_val.endsWith("(") || factorial_val.endsWith(")")) {
        if (factorial_val.endsWith("(")) { i -= 1; console.log(i + " i"); }
        if (factorial_val.endsWith(")")) { p -= 1; console.log(p + " p"); }
    }
